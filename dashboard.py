...
def _charts_scout_panel(tf_min, date, as_of_dt, live=False, seen=None, practice=False):
    ...
    title = html.Div([
        ...
        # "band {_hz}m", NOT "hold {_hz}m". The dropdown moves the FORECAST horizon and
        # nothing else: positions are generated by the headless poller at _ALERT_TF and
        # capped at _SCOUT_MAX_HOLD_MIN, neither of which reads this control. Saying
        # "hold" put "hold ≈15m" directly above a row reading "held 72m/90m".
        html.Span(f"  ·  band {_hz}m" + ("  · 1 look/bar" if _tf == _hz else
                                         f"  · {max(1, _hz // _tf)} looks")
                  + (f"  · ⚠ legs are {_ALERT_TF}m" if _hz != _ALERT_TF else ""),
                  title=((f"⚠ THIS CONTROL CHANGES THE READ, NOT THE TRADES.\n"
                          f"The band, cover% and verdict above follow it. The OPEN/CLOSED "
                          f"legs in the ledger do NOT: they are generated by the headless "
                          f"alert poller at {_ALERT_TF}m, exit on the {_ALERT_TF}m band "
                          f"break, and are capped at {_SCOUT_MAX_HOLD_MIN}m — none of which "
                          f"can see a per-browser dropdown. A leg showing 'held 72m/90m' "
                          f"while this says {_hz}m is those two clocks, not a bug.\n\n"
                          if _hz != _ALERT_TF else "")
                         + ("SINGLE FRAME: trigger and horizon are the same bar, so there is no "

...
powershell -NoProfile -Command "$l='logs\verify_nse.log'; if(-not (Test-Path $l) -or (Get-Item $l).LastWriteTime -lt (Get-Date).AddDays(-6)){ Write-Host '[NSE] weekly drift check...'; & '.\verify_nse.bat' }; if(Test-Path 'logs\verify_nse.DRIFT'){ Write-Host ''; Write-Host '  *** NSE CONSTANT DRIFT DETECTED - see logs\verify_nse.log ***' -ForegroundColor Red; Write-Host '' }"

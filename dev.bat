...
powershell -NoProfile -Command "$l='logs\verify_nse.log'; if(-not (Test-Path $l) -or (Get-Item $l).LastWriteTime -lt (Get-Date).AddDays(-6)){ Write-Host '[NSE] weekly drift check...'; & '.\verify_nse.bat' }; $w= if(Test-Path $l){(Get-Item $l).LastWriteTime.ToString('yyyy-MM-dd HH:mm')}else{'never'}; if(Test-Path 'logs\verify_nse.DRIFT'){ Write-Host ''; Write-Host ('  *** NSE CONSTANT DRIFT DETECTED ('+$w+') - see logs\verify_nse.log ***') -ForegroundColor Red; Write-Host '' } else { Write-Host ('[NSE] constants verified OK - holidays/expiry/lots match (last check '+$w+')') -ForegroundColor Green }"
